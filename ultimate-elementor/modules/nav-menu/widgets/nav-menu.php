...
class Nav_Menu extends Common_Widget {
    ...
    $content_type = array(
        ...
        'crocobock_parts' => __( 'Crocobock Theme Part', 'uael' ),
    );
...
/* TEST */
/**
 * Get JetEngine theme parts list.
 *
 * @return array List of theme parts.
 */
/**
 * Get Jet Theme Core sections.
 *
 * @return array List of theme parts.
 */
public function get_crocobock_theme_parts() {
    ...
    // Abfrage für JetThemeCore-Sektionen
    $args = array(
			...
			foreach ( $settings['menu_items'] as $menu => $item ) {
				...
				if ( 'item_submenu' === $item['item_type'] ) {
					...
					if ( 'sub_menu' === $item['menu_content_type'] ) {
						...
						} elseif ( 'crocobock_parts' === $item['menu_content_type'] ) {
									$theme_part_id = $item['content_crocobock_parts'];
									
									if ( '-1' !== $theme_part_id ) {
										// Verwende Elementor, um den Inhalt zu rendern
										if (class_exists('\Elementor\Plugin')) {
											$section_content = \Elementor\Plugin::$instance->frontend->get_builder_content_for_display($theme_part_id);
											
											// Wir umschließen den Inhalt mit einem Div, um bessere Kontrolle zu haben
											$output .= '<div class="uael-nav-menu-jet-section jet-section-' . esc_attr($theme_part_id) . '">';
											$output .= $section_content;
											$output .= '</div>';
										} else {
											// Fallback, falls Elementor nicht verfügbar ist
											$output .= '<div class="uael-nav-menu-jet-section-error">Elementor ist erforderlich, um diesen Inhalt anzuzeigen.</div>';
										}
									}
								}
						/* mein code - ende */
